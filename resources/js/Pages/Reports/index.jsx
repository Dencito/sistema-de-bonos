...
export default function Reports({ auth, branches, shifts, filters }) {
    ...
    return (
      <div className="grid grid-cols-1 gap-6 md:grid-cols-3">
        <div className="w-full">
          <label className="block mb-1 text-sm font-medium text-gray-700">Turno</label>
          <select
            value={shiftId}
            onChange={(e) => {
              ...
              if (selectedShiftId) {
                const selectedShift = shifts.find(
                  (shift) => shift.id.toString() === selectedShiftId,
                );
                if (selectedShift) {
                  setStartDate(new Date(selectedShift.opening_time));
                  setEndDate(
                    selectedShift.closing_time ? new Date(selectedShift.closing_time) : new Date(),
                  );
                }
              } else {
        ...
        <div className="w-full">
          <label className="block mb-1 text-sm font-medium text-gray-700">RUT/Código</label>
          <input
            type="text"
    ...
    return (
      <div className="flex flex-wrap gap-4 mb-4">
        <div className="w-full md:w-auto">
          <label className="block mb-1 text-sm font-medium text-gray-700">Tipo de reporte</label>
          <select
            value={reportType}
        ...
        <div className="p-6">
          <div className="mb-8">
            ...
            {reportData.summary.ticket_types &&
              Object.keys(reportData.summary.ticket_types).length > 0 && (
                <div className="mt-6">
                  <h4 className="mb-3 font-semibold text-md">Desglose por tipo de ticket</h4>
                  <div className="space-y-2">
                    {Object.entries(reportData.summary.ticket_types).map(([type, data]) => (
                      <div key={type} className="flex justify-between p-3 bg-gray-50 rounded-lg">
                        <span className="font-medium">{type}</span>
                        <span className="text-gray-600">
                          {data.count} tickets - ${data.amount}
                        </span>
                      </div>
                    ))}
                  </div>
          ...
          {reportType === 'top' && reportData.top_players ? (
            <div className="overflow-x-auto">
              <h3 className="mb-4 text-lg font-semibold">Top Jugadores</h3>
                    ...
                    <tr key={`${player.user.id}-${index}`}>
                      <td className="px-6 py-4 whitespace-nowrap">
                        {player.user.first_name +
                          ' ' +
                          player.user.second_name +
                          ' ' +
                          player.user.first_last_name +
                          ' ' +
                          player.user.second_last_name}
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">{player.ticket_count}</td>
                      <td className="px-6 py-4 whitespace-nowrap">
            ...
            <div className="overflow-x-auto">
              <h3 className="mb-4 text-lg font-semibold">Detalle de Tickets</h3>
              <table className="min-w-full divide-y divide-gray-200">
                <thead className="bg-gray-50">
                  <tr>
                    <th className="px-6 py-3 text-xs font-medium tracking-wider text-left text-gray-500 uppercase">
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {reportData.logs
                    ...
                    .map((log) => (
                      <tr key={log.id} className={log.is_player ? 'bg-green-50' : 'bg-purple-50'}>
                        <td className="px-6 py-4 whitespace-nowrap">
                          {log.user?.username ??
      ...
      <div className="bg-white rounded-lg shadow-sm">
        ...
        <div className="p-6">
          ...
          {Object.keys(reportData.summary.by_branch).length > 0 && (
            <div className="mt-6">
              <h4 className="mb-3 font-semibold text-md">Resumen por Sucursal</h4>
              <div className="overflow-x-auto">
                <table className="min-w-full divide-y divide-gray-200">
                  <thead className="bg-gray-50">
                    <tr>
                  ...
                  <tbody className="bg-white divide-y divide-gray-200">
                    {Object.entries(reportData.summary.by_branch).map(
                      ([branchName, data], index) => (
                        <tr key={`branch-${index}`}>
                          <td className="px-6 py-4 whitespace-nowrap">{branchName}</td>
                          <td className="px-6 py-4 whitespace-nowrap">{data.total}</td>
                          <td className="px-6 py-4 whitespace-nowrap">{data.workers}</td>
                          <td className="px-6 py-4 whitespace-nowrap">{data.players}</td>
                        </tr>
                      ),
                    )}
                  </tbody>
        ...
        <div className="p-6 border-t">
          <h3 className="mb-4 text-lg font-medium">Detalle de Marcaciones</h3>
          <div className="overflow-x-auto">
            <table className="min-w-full divide-y divide-gray-200">
              <thead className="bg-gray-50">
                <tr>
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {reportData.logs.map((log) => (
                  <tr
                    key={log.id}
                    className={
                      log.is_player ? 'bg-blue-50' : log.is_worker ? 'bg-purple-50' : 'bg-gray-50'
                    }
                  >
                    <td className="px-6 py-4 whitespace-nowrap">{log.user?.name || 'N/A'}</td>
                    <td className="px-6 py-4 whitespace-nowrap">
                      {log.is_worker ? (
                        <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-purple-100 text-purple-800">
                          Trabajador
  ...
  const renderFingerprintReport = () => {
    ...
    return (
      <div className="bg-white rounded-lg shadow-sm">
        ...
        {/* Detailed Logs */}
        <div className="p-6 border-t">
          <h3 className="mb-4 text-lg font-medium">Registros Detallados</h3>
          <div className="overflow-x-auto">
            <table className="min-w-full divide-y divide-gray-200">
              <thead className="bg-gray-50">
                <tr>
  ...
  const renderShiftReport = () => {
    ...
    return (
      <div className="bg-white rounded-lg shadow-sm">
        ...
        <div className="p-6">
          <div className="mb-8">
            <h3 className="mb-4 text-lg font-semibold">Información del Turno</h3>
          ...
          <div className="mb-8">
            <h3 className="mb-4 text-lg font-semibold">Resumen del Turno</h3>
            <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
              <div className="p-4 bg-gray-50 rounded-lg">
                <p className="text-sm text-gray-600">Total marcaciones</p>
          ...
          <div className="overflow-x-auto">
            <h3 className="mb-4 text-lg font-semibold">Detalle de Marcaciones</h3>
            <table className="min-w-full divide-y divide-gray-200">
              <thead className="bg-gray-50">
                <tr>
                  <th className="px-6 py-3 text-xs font-medium tracking-wider text-left text-gray-500 uppercase">
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {reportData.logs.map((log) => (
                  <tr
                    key={log.id}
                    className={
                      log.is_player ? (log.ticket ? 'bg-green-50' : 'bg-yellow-50') : 'bg-gray-50'
                    }
                  >
                    <td className="px-6 py-4 whitespace-nowrap">
  ...
  return (
    <AuthenticatedLayout
      ...
      role={auth.role}
      auth={auth}
      header={<h2 className="text-xl font-semibold leading-tight text-gray-800">Reportes</h2>}
    >
      <Head title="Reportes" />
