...
export const SelectAssignBonuses = ({ setSelectedRowKeys, selectedRowKeys, bonuses }) => {
  ...
  const handleAssignBonuses = async () => {
    try {
      const response = await bonusService.assignMultipleUsers(selectedRowKeys, selectBonusId);
      if (response.success) {
  ...
  const handleDestroyBonuses = async () => {
    try {
      const response = await bonusService.destroyMultipleUsers(selectedRowKeys, selectBonusId);
      if (response.success) {
        successMsg(response.message);
        router.visit('/users', {
          preserveState: true,
