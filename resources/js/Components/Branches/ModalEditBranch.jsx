      ...
      >
        ...
        >
          ...
          <div className="flex gap-5">
            ...
            >
              <Select
                onChange={() => setCountry(form?.getFieldsValue()?.branchAddressCountry)}
                showSearch
            ...
            <Card title="Horarios Disponibles" size="small">
              ...
              <ul>
                  {availableSchedules?.map((schedule, index) => (
                    <li key={index}>
                      <strong>{schedule?.day}</strong> - {schedule?.ranges?.map((range, rangeIndex) => (
                        <span key={rangeIndex}>
                          {range?.start_time} - {range?.end_time}{rangeIndex < schedule?.ranges?.length - 1 ? ', ' : ''}
                        </span>
                      ))}
                    </li>
                  ))}
