...
const baseUserSchema = {
  username: z.string().min(1, 'El nombre de usuario es requerido').max(15, 'Máximo 15 caracteres'),
...
const trabajadorSchema = z.object({
  ...
  first_last_name: z.string().min(1, 'El primer apellido es requerido').max(20),
...
const jugadorSchema = z.object({
  first_name: z.string().min(1, 'El primer nombre es requerido').max(20, 'Máximo 20 caracteres'),
  ...
  phone: z.string().min(1, 'El número de teléfono es requerido').max(10, 'Máximo 10 caracteres'),
  rutNumbers: z
    .string()
    .min(1, 'Los números del RUT son requeridos')
    .max(10, 'Máximo 10 caracteres'),
  rutDv: z.string().min(1, 'El código de verificación es requerido').max(1, 'Máximo 1 caracter'),
  birth_date: z.string().min(1, 'La fecha de nacimiento es requerida'),
  email: z
    .string()
    .min(1, 'El correo electrónico es requerido')
  ...
  nationality: z.string().min(1, 'La nacionalidad es requerida'),
  address: z.string().min(1, 'La dirección es requerida').max(100, 'Máximo 100 caracteres'),
  branches: z.array(z.number()).min(1, 'Debe seleccionar al menos una sucursal'),
...
export default function ModalViewUser({ data, statuses, roles, branches, userType, categories }) {
                ...
                <FormControl>
                  <Input
                    ...
                    onChange={(e) => {
                      const value = e.target.value.replace(/\D/g, '');
                      field.onChange(value);
                      validateRut(value, form.getFieldValue('rutDv'));
                    }}
                  />
                </FormControl>
                <FormMessage />
                ...
                <FormControl>
                  <Input
                    ...
                    onChange={(e) => {
                      ...
                      field.onChange(value);
                      validateRut(form.getFieldValue('rutNumbers'), value);
                    }}
                  />
                </FormControl>
                <FormMessage />
        ...
        <FormField
          ...
          render={({ field }) => (
            <FormItem>
              <FormLabel>Dirección</FormLabel>
              <FormControl>
                <Input {...field} maxLength={100} placeholder="Ciudad, calle, numero y ETC." />
              </FormControl>
              <FormMessage />
            </FormItem>
          )}
        ...
        <FormField
          ...
          render={({ field }) => {
            const selectedBranches =
              field.value?.map((id) => branches?.find((b) => b.id === id)?.name).filter(Boolean) ||
              [];
                ...
                <FormControl>
                  ...
                  >
                    ...
                    <MultiSelectorContent>
                      <MultiSelectorList>
                        {branches?.map((branch) => (
                          <MultiSelectorItem key={branch.id} value={branch.name}>
                            {branch.name}
                          </MultiSelectorItem>
                        ))}
                      </MultiSelectorList>
      ...
      <Sheet open={isOpen} onOpenChange={setIsOpen}>
        ...
        <SheetContent className="w-full sm:min-w-[700px]">
          <SheetHeader>
            ...
            <SheetDescription>Visualiza y edita la información del usuario</SheetDescription>
          </SheetHeader>
          <Form {...form}>
      ...
      <Dialog open={showConfirmDialog} onOpenChange={setShowConfirmDialog}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Hay cambios sin guardar. ¿Estás seguro de que quieres cerrar?
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
